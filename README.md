...
这是一个用于从GitHub搜索和验证多种AI服务API密钥的Go语言版本项目。  
基于 [GakkiNoOne/hajimi-king](https://github.com/GakkiNoOne/hajimi-king) 项目重构，提供更好的性能和并发处理能力。  
支持OpenRouter、SiliconFlow和Gemini API密钥验证，默认使用OpenRouter进行验证。

⚠️ **注意**：本项目正处于beta期间，功能、结构、接口等都有可能变化，不保证稳定性，请自行承担风险。

## 🚀 核心功能

1. **🔍 GitHub搜索AI API密钥** - 基于自定义查询表达式搜索GitHub代码中的OpenRouter、SiliconFlow和Gemini API密钥
...
强烈建议使用！GITHUB、OPENROUTER、SILICONFLOW 访问长时间高频都会BAN IP

如果需要使用代理访问GitHub、OpenRouter或SiliconFlow API，推荐使用本地WARP代理：
...
| `SILICONFLOW_CHECK_MODEL` | `deepseek-ai/deepseek-chat` | 用于验证key有效的SiliconFlow模型 🤖 |
